...
Codex CLI 작업 시 참고 가이드.

## 프로젝트 개요

Unity 6 (`6000.3.15f1`) 미니게임. *Exit 8*풍 이상현상 탐지 워킹 루프 1인칭 장르. 설계 단일 진실: `docs/GDD_8번출구류_미니게임.docx` — `.docx` 바이너리, Codex 직접 읽기 불가. 필요 섹션 사용자에게 붙여넣기 요청.
...
- **CSharpier 1.2.6 PostToolUse 훅**: `apply_patch`(=Edit/Write) 직후 `.codex/hooks/format-csharp.ps1` 자동 실행, `Assets/*.cs` 포맷. 직접 호출 불필요.
...
## 권한·샌드박스 정책 (`.codex/config.toml` + `.codex/hooks.json`)

Codex는 Claude식 글롭 deny-list 없음 → 샌드박스 + 승인 + PreToolUse 훅으로 대체.

- **샌드박스** (`config.toml`): `sandbox_mode = "workspace-write"` — 워크스페이스 안에서만 쓰기, `network_access = false`(네트워크 차단), 시스템 경로 차단.
- **승인** (`config.toml`): `approval_policy = "on-request"` — 워크스페이스 밖 동작·승격 필요 시 사용자 승인.
- **편집 가드 훅** (`.codex/hooks/guard-edits.ps1`, PreToolUse): `.unity`/`.meta`/`.csproj`/`.sln`/`.slnx` 직접 편집 + 파괴적 Bash(`rm -rf`, `Remove-Item -Recurse -Force`) deny.
- **`.unity`/`.meta`는 Unity Editor 안에서만** 편집. Codex 텍스트 직접 수정 금지. 가드 훅이 1차 차단하나 best-effort — commit 전 `git status`로 재확인.
...
`docs/GDD_8번출구류_미니게임.docx` 게임 디자인 단일 진실. Codex `.docx` 직접 읽기 불가 — 위 요약에 없는 디테일 필요 시 사용자에게 해당 섹션 붙여넣기 요청.