...
- **판정**: `되돌아감` 트리거 + anomaly 활성 → 정답 `+1`; 그 외 오답 `-1` (최소 0). `전진함` 트리거는 위 로직 반전.
...
- VS Code 기본 솔루션은 `minigame-project-HanuYat.slnx`(`.vscode/settings.json` 지정). 루트의 `minigame-project.slnx`는 잔존 파일이므로 HanuYat 쪽을 사용.
...
- **설치된 Unity AI/NavMesh 패키지**: `com.unity.ai.assistant` 2.8.0-pre.1, `com.unity.ai.inference` 2.6.1, `com.unity.ai.navigation` 2.0.12.
- **단일 씬**: `Assets/Scenes/Main Scene.unity`. 새 씬 추가 시 Build Settings에 등록.
